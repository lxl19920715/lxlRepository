...
public class JobService  {
    ...
    public void startJob(XJob xJob) throws XtlExceptions, KettleException {
        // 运行中的任务，禁止运行，等待结束后可再次运行
        boolean isRun = xJobSubmit.isRun(xJob.getJobId()) ;
        if(isRun){
            log.info("作业名称：" + xJob.getJobName() + "正在运行,等待运行完成后可再次运行!");
            return;
        }
        ...
        if (StringUtils.isEmpty(jobType)) {
            try {
                // 运行完成后删除记录
                XJobSubmit.jobRuns.remove(xJob.getJobId());
    ...
    private void doRunRepoJob(XJob xJob) throws XtlExceptions, KettleException {
        EmailUtils.emailType = xJob.getIsEmail();
        EmailUtils.userId = xJob.getUserId();
        EmailUtils.jobDescription = xJob.getJobDescription();
        EmailUtils.kjbPath = xJob.getJobPath();
        ...
        try {
            ...
            // 作业结束后删除map中的缓存
            XLogListener.kettleLogListenerMap.remove(job);
            // 运行完成后删除记录
            XJobSubmit.jobRuns.remove(xJob.getJobId());
        }
