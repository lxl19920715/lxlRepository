...
//    }
    ...
    public BaseResult getJob(@RequestParam String jobId) {
        ...
        // 发送邮件
        if ("1".equals(xJob.getIsEmail())) {
            // 查询发送邮件表数据
            XJobMailExample xJobMailExample = new XJobMailExample();
            xJobMailExample.createCriteria().andJobIdEqualTo(xJob.getJobId());
            XJobMail xJobMail = this.xJobMailService.selectFirstExample(xJobMailExample);
            dataMap.put("beginTime", xJobMail.getBeginTime());
            dataMap.put("endTime", xJobMail.getEndTime());
        }
    ...
    public BaseResult addJob(@RequestBody @Validated XJob xJob, BindingResult BindingResult) {
        ...
        String isEmail = xJob.getIsEmail();
        ...
        int affect = this.xJobService.insertSelective(xJob);
        if ("1".equals(xJob.getIsEmail())) {
            insertJobEmail(xJob.getJobId(),xJob.getBeginTime(),xJob.getEndTime());
        }
    ...
    public BaseResult updateJob(@RequestBody @Validated Map params, BindingResult BindingResult) {
        ...
        xJob.setJobDescription(params.get("jobDescription")+"");
        xJob.setJobPath(params.get("jobPath")+"");
        xJob.setUserId(params.get("userId")+"");
        int affect = this.xJobService.updateByExampleSelective(xJob, xJobExample);

        // 删除作业邮件数据
        XJobMailExample xJobMailExample = new  XJobMailExample();
        xJobMailExample.createCriteria().andJobIdEqualTo(params.get("jobId") + "");
        this.xJobMailService.deleteByExample(xJobMailExample);
        if ("1".equals(xJob.getIsEmail())){
            // 增加作业邮件数据
            insertJobEmail(params.get("jobId") + "",params.get("beginTime")+"",params.get("endTime")+"");
        }
        if (affect > 0) {
    ...
    public BaseResult delFromLocal(@RequestParam String jobId) {
        try {
            ...
            // 删除作业邮件数据
            XJobMailExample xJobMailExample = new  XJobMailExample();
            xJobMailExample.createCriteria().andJobIdEqualTo(jobId);
            this.xJobMailService.deleteByExample(xJobMailExample);
            if (affect > 0) {
     ...
     * @param endTime
     */
    private void insertJobEmail(String jobId,String beginTime,String endTime){
        XJobMail xJobMail = new XJobMail();
        xJobMail.setJobId(jobId);
        SimpleDateFormat sbf = new SimpleDateFormat("HH:mm:ss");
        Date beginDate = null;// 发送邮件开始时间
        Date endData = null;// 发送邮件结束时间
        try {
            beginDate = sbf.parse(beginTime);
            endData = sbf.parse(endTime);
            xJobMail.setBeginTime(new Time(beginDate.getTime()));
            xJobMail.setEndTime(new Time(endData.getTime()));
